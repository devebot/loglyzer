...
var Constructor = function Constructor(kwargs) {
  ...
  this.process = function() {
    ...
    var _process = function() {
      if (_deque.length > 0) {
        _ec = 0;
        dbx.enabled && dbx('step[%s] + process the next packet', _ec);
        var packet = _deque.pop();
        var flow = Promise.resolve(true);
        if (kwargs.preterior) {
          flow = flow.then(function() {
            dbx.enabled && dbx('step[%s] - before preterior()', _ec);
            return kwargs.preterior(packet, _accumulator);
          });
        }
        flow = flow.then(function(continued) {
          dbx.enabled && dbx('step[%s] - before interceptors, continued? %s', _ec, continued);
        ...
        if (kwargs.posterior) {
          flow = flow.then(function(continued) {
            dbx.enabled && dbx('step[%s] - before posterior, continued? %s', _ec, continued);
            if (continued === false) return continued;
            return kwargs.posterior(packet, _accumulator);
          });
        }
        flow = flow.then(function(continued) {
          dbx.enabled && dbx('step[%s] - loop or exit? %s', _ec, continued);
